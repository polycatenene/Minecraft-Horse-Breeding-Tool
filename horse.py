...
class Stable:
    ...
    def probability(self, horse1, horse2):
        ...
        print(" Probability calculation:",
              f"target HP {self.target.hp: .1f}: {odds_hp * 100 if odds_hp > 0 else 0:.3g}%" ,
              f"target JUMP {self.target.jump:.2f}: {odds_jump * 100 if odds_jump > 0 else 0:.3g}%" ,
              f"target SPEED {self.target.speed:.2f}: {odds_speed * 100 if odds_speed > 0 else 0:.3g}%" ,
